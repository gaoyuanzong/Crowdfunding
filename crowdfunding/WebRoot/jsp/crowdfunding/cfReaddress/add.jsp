...
<head>
	...
	<script type="text/javascript">
		function cfReaddress_add_submit(dialog){
			$("#crowdfunding_cfReaddress_add_form").submit();
		}
		
		$(document).ready(function(){
			 $("#crowdfunding_cfReaddress_add_form").validate({
			 	submitHandler : function(frm){
					commonFrmSubmit(
							"#crowdfunding_cfReaddress_add_form",
							basePath+"admin/crowdfunding/cfReaddress!doAdd.action",
							function(json){
								if(json.success){
									home_fresh();
								}else{
...
<body>
 	<form id="crowdfunding_cfReaddress_add_form" name="crowdfunding_cfReaddress_add_form" class="l-form" method="post">
	 <input type="hidden" name="cfReaddress.id" value="${cfReaddress.id}"/>
		...
		<div style="margin-top:10px;margin-bottom:15px;">
			<a class="l-button" onclick="cfReaddress_add_submit()">提交</a>
			<a class="l-button" onclick="$("#crowdfunding_cfReaddress_add_form").reset()">重置</a>
		</div>
		-->
    </form>
</body>
