...
<head>
	...
	<script type="text/javascript">
		function cfInformation_add_submit(dialog){
			$("#crowdfunding_cfInformation_add_form").submit();
		}
		
		$(document).ready(function(){
			 $("#crowdfunding_cfInformation_add_form").validate({
			 	submitHandler : function(frm){
					commonFrmSubmit(
							"#crowdfunding_cfInformation_add_form",
							basePath+"admin/crowdfunding/cfInformation!doAdd.action",
							function(json){
								if(json.success){
									home_fresh();
								}else{
...
<body>
 	<form id="crowdfunding_cfInformation_add_form" name="crowdfunding_cfInformation_add_form" class="l-form" method="post">
	 <input type="hidden" name="cfInformation.id" value="${cfInformation.id}"/>
		...
		<div style="margin-top:10px;margin-bottom:15px;">
			<a class="l-button" onclick="cfInformation_add_submit()">提交</a>
			<a class="l-button" onclick="$("#crowdfunding_cfInformation_add_form").reset()">重置</a>
		</div>
		-->
    </form>
</body>
