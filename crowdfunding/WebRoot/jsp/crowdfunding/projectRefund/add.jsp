...
<head>
	...
	<script type="text/javascript">
		function projectRefund_add_submit(dialog){
			$("#crowdfunding_projectRefund_add_form").submit();
		}
		
		$(document).ready(function(){
			 $("#crowdfunding_projectRefund_add_form").validate({
			 	submitHandler : function(frm){
					commonFrmSubmit(
							"#crowdfunding_projectRefund_add_form",
							basePath+"admin/crowdfunding/projectRefund!doAdd.action",
							function(json){
								if(json.success){
									home_fresh();
								}else{
...
<body>
 	<form id="crowdfunding_projectRefund_add_form" name="crowdfunding_projectRefund_add_form" class="l-form" method="post">
	 <input type="hidden" name="projectRefund.id" value="${projectRefund.id}"/>
		...
		<div style="margin-top:10px;margin-bottom:15px;">
			<a class="l-button" onclick="projectRefund_add_submit()">提交</a>
			<a class="l-button" onclick="$("#crowdfunding_projectRefund_add_form").reset()">重置</a>
		</div>
		-->
    </form>
</body>
