...
<head>
	...
	<script type="text/javascript">
		function projectProgress_add_submit(dialog){
			$("#crowdfunding_projectProgress_add_form").submit();
		}
		
		$(document).ready(function(){
			 $("#crowdfunding_projectProgress_add_form").validate({
			 	submitHandler : function(frm){
					commonFrmSubmit(
							"#crowdfunding_projectProgress_add_form",
							basePath+"admin/crowdfunding/projectProgress!doAdd.action",
							function(json){
								if(json.success){
									home_fresh();
								}else{
...
<body>
 	<form id="crowdfunding_projectProgress_add_form" name="crowdfunding_projectProgress_add_form" class="l-form" method="post">
	 <input type="hidden" name="projectProgress.id" value="${projectProgress.id}"/>
		...
		<div style="margin-top:10px;margin-bottom:15px;">
			<a class="l-button" onclick="projectProgress_add_submit()">提交</a>
			<a class="l-button" onclick="$("#crowdfunding_projectProgress_add_form").reset()">重置</a>
		</div>
		-->
    </form>
</body>
