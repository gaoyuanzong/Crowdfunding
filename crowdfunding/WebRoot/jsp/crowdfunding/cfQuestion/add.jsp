...
<head>
	...
	<script type="text/javascript">
		function cfQuestion_add_submit(dialog){
			$("#crowdfunding_cfQuestion_add_form").submit();
		}
		
		$(document).ready(function(){
			 $("#crowdfunding_cfQuestion_add_form").validate({
			 	submitHandler : function(frm){
					commonFrmSubmit(
							"#crowdfunding_cfQuestion_add_form",
							basePath+"admin/crowdfunding/cfQuestion!doAdd.action",
							function(json){
								if(json.success){
									home_fresh();
								}else{
...
<body>
 	<form id="crowdfunding_cfQuestion_add_form" name="crowdfunding_cfQuestion_add_form" class="l-form" method="post">
	 <input type="hidden" name="cfQuestion.id" value="${cfQuestion.id}"/>
		...
		<div style="margin-top:10px;margin-bottom:15px;">
			<a class="l-button" onclick="cfQuestion_add_submit()">提交</a>
			<a class="l-button" onclick="$("#crowdfunding_cfQuestion_add_form").reset()">重置</a>
		</div>
		-->
    </form>
</body>
